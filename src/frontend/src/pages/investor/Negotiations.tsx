      ...
      <main className="container max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        <div className="space-y-4">
          {mockNegotiations.map((negotiation) => (
            <div
              key={negotiation.id}
              onClick={() => navigate(`/investor/negotiation/${negotiation.id}`)}
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                    <span className="text-lg font-semibold">
                      {negotiation.borrowerName.charAt(0)}
                    </span>
                  </div>
                  <div>
                    <p className="font-medium text-foreground">{negotiation.borrowerName}</p>
                    <p className="text-xs text-muted-foreground">Score: {negotiation.borrowerScore}</p>
                  </div>
                </div>
